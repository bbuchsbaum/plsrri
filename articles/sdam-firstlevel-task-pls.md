...
`neuroatlas::plot_brain()`. This is useful for visual comparison, but it
is a projection of the voxel map, not a different statistical test. The
helper uses a neutral grey base surface so the colours encode only the
projected bootstrap-ratio overlay.
