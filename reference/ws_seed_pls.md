...
- beta_lst:
  ...
  columns = voxels. Typically produced by `fmrilss::lss()`.
...
(LSS) via `fmrilss::lss()`, which provides stable per-trial activation
estimates even with rapid event-related designs.
