...
`fmrilss::lss()`) rather than condition means. For each subject and
condition, Pearson correlations are computed between the seed region's
trial-by-trial betas and every voxel's trial-by-trial betas. The
resulting within-subject correlation maps (optionally Fisher-z
transformed) are stacked into a data matrix and analyzed with task PLS.
